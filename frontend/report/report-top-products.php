...
$allowed_roles = ['Admin', 'Manager', 'Sales', 'Accounting'];
...
<head>
  ...
  <title>Top Products Report - PT. Dorareta Indo Makmur</title>
  ...
  <script src="../js/report-top-products.js" defer></script>
</head>
  ...
  <main class="px-4 pt-28 pb-10 sm:px-6 lg:px-8">
    ...
    <div class="w-full mb-5">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2">
        <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">Top Products Report</h2>
        <div class="text-sm text-gray-500 dark:text-gray-300">
          <a href="../dashboard/dashboard.php" class="text-gray-600 dark:text-gray-200 hover:underline">Dashboard</a>
          <span class="mx-1">/</span>
          <span class="text-red-600 font-semibold">Top Products Report</span>
        </div>
      </div>
...
<div class="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 mb-6">
  <!-- Search Field -->
  <div>
    <label for="filterKeyword" class="block text-sm font-medium mb-1">Search</label>
    <div class="flex gap-2">
      <input
        type="text"
        id="filterKeyword"
        placeholder="Search product or category"
        class="w-64 px-3 py-2 border rounded-md bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600 text-sm text-gray-900 dark:text-white"
      />
      <button
        id="resetFilters"
        class="px-3 py-2 rounded-md bg-gray-200 dark:bg-gray-700 text-sm text-gray-800 dark:text-white hover:bg-gray-300 dark:hover:bg-gray-600 transition"
      >
        Reset
      </button>
    </div>
...
<div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-x-auto">
  <table id="topProductsTable" class="min-w-full table-auto text-sm">
    <thead class="bg-gray-100 dark:bg-gray-700 text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
      <tr>
        <th class="px-3 py-2 text-left whitespace-nowrap">Product</th>
        <th class="px-3 py-2 text-center whitespace-nowrap">Sold Qty</th>
        <th class="px-3 py-2 text-right whitespace-nowrap">Total Sales</th>
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="relative">
          <select id="rowsPerPage"
            class="appearance-none px-2 py-1 pr-8 rounded-md bg-transparent border border-transparent text-gray-800 dark:text-white focus:outline-none focus:ring-0 focus:border-transparent [&>option]:text-black cursor-pointer">

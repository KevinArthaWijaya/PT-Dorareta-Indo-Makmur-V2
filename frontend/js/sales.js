document.addEventListener("DOMContentLoaded", () => {
  const userRole = window.userRole || ""; // Sesuaikan ini agar sesuai dengan inject PHP

  if (!["Admin", "Sales"].includes(userRole)) {
    // 🔒 Sembunyikan tombol create
    document.getElementById("openCreateSale")?.classList.add("hidden");

    // 🔒 Sembunyikan tombol return di modal detail
    document
      .querySelectorAll(".returnBtn")
      .forEach((btn) => btn.classList.add("hidden"));

    // 🔒 Nonaktifkan fitur submit sales
    document.getElementById("submitSalesBtn")?.classList.add("hidden");

    // 🔒 Sembunyikan tombol delete/edit di tabel
    document.addEventListener("DOMSubtreeModified", () => {
      document
        .querySelectorAll(".editSalesBtn, .deleteSalesBtn")
        .forEach((btn) => {
  ...
  const openSalesBtn = document.getElementById("openCreateSale");
  ...
  const grandTotalDisplay = document.getElementById("grandTotalDisplaySale");
  const paidInput = document.getElementById("paid");
  const dueInput = document.getElementById("due");
  const paymentStatus = document.getElementById("payment_status");
  const addProductRowBtn = document.getElementById("addProductRowSale");
  ...
  flatpickr("#filterSalesStartDate", {
    dateFormat: "Y-m-d",
    maxDate: "today",
  });
  flatpickr("#filterSalesEndDate", {
    dateFormat: "Y-m-d",
    maxDate: "today",
  });
  ...
  openSalesBtn.addEventListener("click", () => {
    ...
    modalTitle.textContent = "Create Sales";
    submitBtn.textContent = "Save Sales";
  ...
  closeSalesBtn.addEventListener("click", closeModal);
  ...
  async function fetchAndRenderSaless() {
    try {
      ...
      url.searchParams.set("path", "sales");
      url.searchParams.set("page", currentPageSales);
      url.searchParams.set("limit", rowsPerPageSales);
      ...
      result.data.forEach((sales) => {
        const tr = document.createElement("tr");
        const totalItems = sales.items?.length || 0;

        tr.innerHTML = `
        <td class="px-4 py-2">${sales.invoice_no}</td>
        <td class="px-4 py-2">${sales.date}</td>
        <td class="px-4 py-2">${sales.customer_name || "-"}</td>
        <td class="px-4 py-2">${totalItems}</td>
        <td class="px-4 py-2 text-center">${getStatusBadge(sales.status)}</td>
        <td class="px-4 py-2 text-right">Rp ${Number(
          sales.grand_total
        ).toLocaleString("id-ID")}</td>
        <td class="px-4 py-2 text-right">Rp ${Number(sales.paid).toLocaleString(
          "id-ID"
        )}</td>
        <td class="px-4 py-2 text-right">Rp ${Number(sales.due).toLocaleString(
          "id-ID"
        )}</td>
      ...
      renderPaginationSales(result.total, result.page, result.total_pages);
      populateFilterDropdowns();
    } catch (err) {
      console.error("Gagal memuat sales:", err);
    }
  }

  function renderPaginationSales(totalItems, page, totalPages) {
  ...
  document
    .getElementById("salesRowsPerPage")
    ?.addEventListener("change", (e) => {
      rowsPerPageSales = parseInt(e.target.value);
      currentPageSales = 1;
      fetchAndRenderSaless();
    });
  ...
  function populateFilterDropdowns() {
    ...
    document.querySelectorAll("#salesTableBody tr").forEach((row) => {
      ...
      const customer = row.children[2]?.textContent.trim();

      if (invoice) invoiceSet.add(invoice);
      if (customer) customerSet.add(customer);
    });

    const customerSelect = document.getElementById("filterSalesCustomer");
    const invoiceSelect = document.getElementById("filterSalesInvoice");

    customerSelect.innerHTML = '<option value="">-- Semua Customer --</option>';
    invoiceSelect.innerHTML = '<option value="">-- Semua Invoice --</option>';

    [...customerSet].sort().forEach((name) => {
  ...
  salesForm.addEventListener("submit", async (e) => {
    ...
    if (!invoice || !date || !customer) {
      Swal.fire(
        "Oops!",
        "Invoice, Tanggal, dan Customer wajib diisi.",
        "warning"
      );
      return;
    }

    const productRows = salesItemsContainer.querySelectorAll(".grid");
    ...
    const salesId = document.getElementById("sale_id").value;
    if (salesId) {
    ...
    try {
      ...
      if (isEdit) {
        const response = await fetch(
          "../../backend/api/sales/index.php?path=sales&method=PUT",
          {
            method: "POST",
            body: formData,
          }
        ...
        const response = await fetch(
          "../../backend/api/sales/index.php?path=sales",
          {
            method: "POST",
            body: formData,
          }
  ...
  const modalContent = document.getElementById("salesFilterContent");
  ...
  const closeFilterBtn = document.getElementById("closeSalesFilterModalBtn");
  ...
  applyFilterBtn?.addEventListener("click", () => {
    ...
    const startDate =
      document.getElementById("filterSalesStartDate")?.value || "";
    const endDate = document.getElementById("filterSalesEndDate")?.value || "";
    const customer = (
      document.getElementById("filterSalesCustomer")?.value || ""
    ).toLowerCase();
    const invoice = (
      document.getElementById("filterSalesInvoice")?.value || ""
    ).toLowerCase();
    const status = document.getElementById("filterSalesStatus")?.value || "";
    const paymentStatus =
      document.getElementById("filterSalesPaymentStatus")?.value || "";
    const grandMinInput = document.getElementById("filterSalesGrandMin");
    const grandMaxInput = document.getElementById("filterSalesGrandMax");
    const grandMin = getCleanNumber(grandMinInput?.value);
    const grandMax = getCleanNumber(grandMaxInput?.value) || Infinity;
    const minItems =
      parseInt(document.getElementById("filterSalesMinItems")?.value) || 0;
    ...
    rows.forEach((row) => {
      ...
      const rowCustomer = cells[2].textContent.trim().toLowerCase();
      ...
      const matchCustomer = !customer || rowCustomer.includes(customer);
  ...
  clearFilterBtn?.addEventListener("click", () => {
    document
      .querySelectorAll("#salesFilterModal input, #salesFilterModal select")
      .forEach((input) => {
        input.value = "";
      });
  ...
  suggestionBox.id = "customerSuggestions";
  ...
  customerInput.parentElement.appendChild(suggestionBox);
  ...
  customerInput.addEventListener("input", async () => {
    ...
    try {
      const res = await fetch(
        `../../backend/api/sales/index.php?path=search_customers&q=${encodeURIComponent(
          keyword
        )}`
      );
  ...
  function addProductRow(name = "", qty = 1, cost = 0, subtotal = 0) {
    ...
    row.innerHTML = `
        ...
        <div class="col-span-2">
          <input type="text" name="unit_price[]" value="${cost}" readonly tabindex="-1"
            class="w-full p-2 text-center border rounded-md bg-gray-100 dark:bg-gray-600 border-gray-300 dark:border-gray-500 text-black dark:text-white cursor-not-allowed cost-input" />
        </div>
  ...
  function updateCalculation() {
    const rows = salesItemsContainer.querySelectorAll(".grid");
    let grandTotal = 0;
  ...
  function enableProductAutocomplete(input) {
    ...
    input.addEventListener("input", async () => {
      ...
      try {
        const res = await fetch(
          `../../backend/api/sales/index.php?path=search_products&q=${encodeURIComponent(
            keyword
          )}`
        );
        const data = await res.json();
        ...
        if (data.success && Array.isArray(data.data) && data.data.length > 0) {
          data.data.forEach((item) => {
            ...
            const minimal_purchase = item.minimal_purchase || 1;
            ...
            li.addEventListener("click", () => {
              ...
              if (row) {
                ...
                if (costInput) {
                  costInput.value = selling_price.toLocaleString("id-ID");
                }

                if (qtyInput) {
                  const currentVal = parseInt(qtyInput.value) || 0;
                  if (currentVal < minimal_purchase) {
                    qtyInput.value = minimal_purchase;
                  }
                }
  ...
  document.addEventListener("click", async (e) => {
    if (e.target.closest(".editSalesBtn")) {
      const id = e.target.closest(".editSalesBtn").dataset.id;

      try {
        const res = await fetch(
          `../../backend/api/sales/index.php?path=sales&id=${id}`
        );
        const result = await res.json();
        if (!result.success)
        ...
        submitBtn.textContent = "Update Sales";
        ...
        document.getElementById("sale_date")._flatpickr.setDate(data.date);
  ...
  document.addEventListener("click", async (e) => {
    if (e.target.closest(".deleteSalesBtn")) {
      const id = e.target.closest(".deleteSalesBtn").dataset.id;
  ...
  //VIEW SALES
  document.addEventListener("click", async (e) => {
    if (e.target.closest(".viewSalesBtn")) {
      const id = e.target.closest(".viewSalesBtn").dataset.id;
      ...
      try {
        ...
        const data = result.data;
        document.querySelector(".returnBtn").setAttribute("data-id", data.id);

        // 🔄 Set info
        document.getElementById("view_sales_date").textContent = data.date;
        document.getElementById("view_sales_invoice").textContent =
          data.invoice_no;
        document.getElementById("view_sales_customer").textContent =
          data.customer_name || "-";
        document.getElementById(
          "view_sales_grand_total"
        ).textContent = `Rp${Number(data.grand_total).toLocaleString("id-ID")}`;
        document.getElementById("view_sales_paid").textContent = `Rp${Number(
          data.paid
        ).toLocaleString("id-ID")}`;
        document.getElementById("view_sales_due").textContent = `Rp${Number(
          data.due
        ).toLocaleString("id-ID")}`;
        const statusBadge = document.getElementById("view_sales_status");
        const paymentBadge = document.getElementById(
          "view_sales_payment_status"
        );
        ...
        // 🔄 Isi tabel item
        const tbody = document.getElementById("viewSalesItems");
        tbody.innerHTML = "";
        ...
        const modalContent = document.getElementById("viewSalesContent");
  ...
  const closeViewBtn = document.getElementById("closeViewSalesModal");
  ...
  //Logika Button Print Sales Detail
  ...
  @media print {
    ...
    const invoice =
      clone.querySelector("#view_sales_invoice")?.textContent || "";
    const date = clone.querySelector("#view_sales_date")?.textContent || "";
    const customer =
      clone.querySelector("#view_sales_customer")?.textContent || "";
    const status = clone.querySelector("#view_sales_status")?.textContent || "";
    const payStatus =
      clone.querySelector("#view_sales_payment_status")?.textContent || "";
    const grandTotal =
      clone.querySelector("#view_sales_grand_total")?.textContent || "";
    const paid = clone.querySelector("#view_sales_paid")?.textContent || "";
    const due = clone.querySelector("#view_sales_due")?.textContent || "";
    const tbody = clone.querySelector("#viewSalesItems")?.innerHTML || "";
    ...
    <div class="info">
      ...
      <div>
        ...
        <strong>SUPPLIER:</strong> ${customer}
      </div>
    </div>
    ...
    printWindow.document.write(
      `<html><head><title>Print Sales</title>${style}</head><body>${html}</body></html>`
    );
  ...
  document.addEventListener("click", (e) => {
    ...
    if (returnBtn) {
      const viewModal = document.getElementById("viewSalesModal");
      const returnModal = document.getElementById("returnSalesModal");

      if (viewModal) viewModal.classList.add("hidden");
      if (returnModal) returnModal.classList.remove("hidden");

      document.getElementById("return_sales_items").innerHTML = "";
      returnProductOptions = [];

      // Isi sales info dari modal view sales
      document.getElementById("return_sale_date").textContent =
        document.getElementById("view_sales_date").textContent;
      document.getElementById("return_sale_status").textContent =
        document.getElementById("view_sales_status").textContent;
      document.getElementById("return_sale_invoice").textContent =
        document.getElementById("view_sales_invoice").textContent;
      document.getElementById("return_sale_payment_status").textContent =
        document.getElementById("view_sales_payment_status").textContent;
      document.getElementById("return_sale_customer").textContent =
        document.getElementById("view_sales_customer").textContent;
      document.getElementById("return_sale_id").value =
        returnBtn.getAttribute("data-id") || "";

      // Ambil data produk dari modal view sales dan simpan di returnProductOptions
      const rows = document.querySelectorAll("#viewSalesItems tr");
      rows.forEach((tr) => {
    ...
    if (e.target?.id === "addReturnSalesRow") {
      addReturnProductRow();
    }
  ...
  function addReturnProductRow() {
    const tbody = document.getElementById("return_sales_items");
  ...
  function attachReturnRowListeners(row) {
    ...
    productSelect.addEventListener("change", () => {
      ...
      row.querySelector(".return-cost").value = isNaN(price)
        ? "Rp0"
    ...
    updateReturnTotal(); // call once on row attach
  }

  function updateReturnTotal() {
    const rows = document.querySelectorAll("#return_sales_items .grid");
    let total = 0;
    ...
    const totalDisplay = document.getElementById("return_sales_grand_total");
    if (totalDisplay) {
  ...
  document
    .getElementById("returnSalesForm")
    ?.addEventListener("submit", async (e) => {
      e.preventDefault();

      const invoice =
        document.getElementById("return_sale_invoice")?.textContent?.trim() ||
        "";
      const customer =
        document.getElementById("return_sale_customer")?.textContent?.trim() ||
        "";
      const status =
        document
          .getElementById("return_sale_status")
          ?.textContent?.trim()
          .toLowerCase() || "";
      const paymentStatus =
        document
          .getElementById("return_sale_payment_status")
          ?.textContent?.trim() || "";
      const salesId = document.getElementById("return_sale_id")?.value?.trim();

      const items = document.querySelectorAll("#return_sales_items .grid");
      const returnData = new FormData();

      if (!invoice || !customer || items.length === 0 || !salesId) {
      ...
      returnData.append("sale_id", salesId);
      returnData.append("invoice_no", invoice);
      returnData.append("customer_name", customer);
      ...
      try {
        const res = await fetch(
          "../../backend/api/sales/index.php?path=sales_return",
          {
            method: "POST",
            body: returnData,
          }
        ...
        // Jika response kosong atau bukan JSON, parse ini akan error
        const result = JSON.parse(text);

        if (result.success) {
          Swal.fire("Sukses", "Return berhasil disimpan", "success");
          document.getElementById("returnSalesModal").classList.add("hidden");
        } else {
  ...
  // Tombol X (closeReturnSalesModalBtn) hanya menutup modal return
  document
    .getElementById("closeReturnSalesModalBtn")
    ?.addEventListener("click", () => {
      document.getElementById("returnSalesModal")?.classList.add("hidden");
    });

  // Tombol Cancel: tutup modal return & buka kembali modal view sales
  document
    .querySelector(".cancel-return-sales-btn")
    ?.addEventListener("click", () => {
      document.getElementById("returnSalesModal")?.classList.add("hidden");
      document.getElementById("viewSalesModal")?.classList.remove("hidden");
    });

  // Sorting (tetap seperti sebelumnya)
  ...
  function sortSalesTable(column, order) {
    const tbody = document.getElementById("salesTableBody");
    ...
    rows.sort((a, b) => {
      ...
      const alphaColumns = ["customer", "status", "payment_status"];
  ...
  searchInput.addEventListener("input", () => {
    ...
    rows.forEach((row) => {
      ...
      const isMatch = invoice.includes(keyword) || customer.includes(keyword);
  ...
  async function generateAutoInvoiceNo() {
    try {
      const res = await fetch(
        "../../backend/api/sales/index.php?path=generate_invoice",
        {
          credentials: "include", // ⬅️ WAJIB agar session terbaca oleh PHP
          headers: {
            Accept: "application/json", // Opsional, tapi baik untuk eksplisitkan response
          },
        }
      );
      ...
      if (
        ...
        const text = await res.text();
        throw new Error("Invalid response (bukan JSON): " + text.slice(0, 100));
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.message || "Gagal mendapatkan nomor invoice.");
      }
    } catch (err) {
      ...
      Swal.fire({
        icon: "error",
        title: "Invoice Error",
        text: err.message || "Gagal membuat invoice otomatis.",
      });
    }
  }
});

document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchPurchaseReport() {
    fetch("../../backend/api/report/purchase.php")
      .then((res) => res.json())
      .then((data) => {
        allData = data;
        renderTable();
  ...
  function renderTable() {
    const keyword = filterSupplier.value.toLowerCase();
    const selectedDate = filterDate.value;
    const selectedStatus = filterStatus.value;

    let filtered = allData.filter((item) => {
      const dateMatch = !selectedDate || item.date === selectedDate;
      const supplierMatch = item.supplier?.toLowerCase().includes(keyword);
    ...
    paginated.forEach((item) => {
      ...
      row.innerHTML = `
          ...
          <td class="px-4 py-2">${item.invoice_no}</td>
          <td class="px-4 py-2">${item.supplier || "-"}</td>
          <td class="px-4 py-2 text-center">${statusBadge(item.status)}</td>
          <td class="px-4 py-2 text-right">Rp${formatNumber(item.total)}</td>
<td class="px-4 py-2 text-right">Rp${formatNumber(item.paid)}</td>
<td class="px-4 py-2 text-right">Rp${formatNumber(item.due)}</td>
<td class="px-4 py-2 text-center">${paymentBadge(item.payment_status)}</td>
        `;
  ...
  function statusBadge(status) {
    ...
    return `<span class="inline-block text-center px-3 py-1 rounded-md text-xs font-semibold capitalize ${color}">${status}</span>`;
  }
  ...
  [filterSupplier, filterStatus].forEach((el) => {
  ...
  // Sorting header
  document
    .querySelectorAll("#purchaseReportTable thead th[data-sort]")
    .forEach((th) => {
  ...
  exportExcelBtn.addEventListener("click", () => {
    ...
    const ws = XLSX.utils.table_to_sheet(
      document.getElementById("purchaseReportTable")
    );
  ...
  printBtn.addEventListener("click", () => {
    const table = document.getElementById("purchaseReportTable").outerHTML;
    ...
    const html = `
        <header>
          ...
          <h1>Laporan Pembelian<br><small style="font-weight: normal">Dicetak: ${date}</small></h1>
        </header>
        ${table}
      `;
    ...
    win.document.write(
      `<html><head><title>Cetak Laporan Pembelian</title>${style}</head><body>${html}</body></html>`
    );
    win.document.close();
    win.focus();
    win.print();

...
$allowed_roles = ['Admin', 'Sales', 'Manager', 'Accounting'];
...
<head>
  <meta charset="UTF-8" />
  <title>Sales Management - PT. Dorareta Indo Makmur</title>
  ...
  <main class="px-4 pt-20 pb-10 sm:px-6 lg:px-8">
    ...
    <div class="w-full">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2 mb-5">
        <h2 class="text-2xl font-semibold text-gray-900 dark:text-white">Sales Management</h2>
        <div class="text-sm text-gray-500 dark:text-gray-300">
          ...
          <span class="text-red-600 font-semibold">Sales Management</span>
        </div>
      </div>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
      ...
      <div class="flex flex-wrap gap-3 w-full sm:w-auto">
        <button id="openCreateSale"
          ...
          <span class="relative z-10">Create Sale</span>
...
<div class="overflow-x-auto bg-white dark:bg-gray-800 shadow rounded-lg">
  <table id="salesTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
    <thead class="bg-gray-100 dark:bg-gray-700 text-xs font-semibold uppercase text-gray-600 dark:text-gray-300 tracking-wider">
      <tr class="divide-x divide-gray-200 dark:divide-gray-600">
        ...
        <th class="px-4 py-3 text-left cursor-pointer sortable" data-column="customer">
          Customer
          <img src="../../assets/icons/lightmode/sort-alt-light.png" class="sort-icon-light w-3 h-3 inline-block ml-1 dark:hidden" />
          <img src="../../assets/icons/darkmode/sort-alt-dark.png" class="sort-icon-dark w-3 h-3 hidden ml-1 dark:inline-block" />
        </th>
    ...
    <tbody id="salesTableBody" class="divide-y divide-gray-200 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
      <!-- Data dari JS -->
    </tbody>
  </table>
</div>
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-6">
  ...
  <!-- Pagination Buttons -->
  <div id="salesPaginationContainer" class="flex items-center gap-2 flex-wrap">
...
<div id="salesModal"
  ...
  <div id="salesModalContent"
    ...
    <div class="flex justify-between items-center mb-4">
      <h3 id="salesModalTitle" class="text-2xl font-semibold text-gray-900 dark:text-white">Create Sale</h3>
      <button id="closeSalesModal" class="text-gray-500 hover:text-red-500 transition">
    ...
    <form id="salesForm" enctype="multipart/form-data" class="space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label for="sale_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Date</label>
          <input type="text" id="sale_date" name="date" required class="w-full mt-1 p-2 border rounded-md bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white" />
        </div>
      ...
      <div class="mt-6 border border-gray-300 dark:border-gray-600 rounded-md overflow-hidden">
        ...
        <div class="px-2 divide-y divide-gray-200 dark:divide-gray-700 max-h-[204px] overflow-y-auto" id="salesItemsContainer">
          <!-- JS generated product rows -->
        </div>

        <div class="px-4 pb-3 pt-2">
          <button type="button" id="addProductRowSale" class="text-sm text-emerald-600 font-medium hover:underline">+ Add Product</button>
        </div>
      </div>
      ...
      <div class="flex justify-end text-lg font-bold text-gray-800 dark:text-white mt-4">
        ...
        <span id="grandTotalDisplaySale">Rp0</span>
      </div>
      ...
      <div class="flex justify-end space-x-3 pt-6">
        <button type="button" id="cancelSalesBtn" class="px-4 py-2 border rounded-md text-gray-700 dark:text-white border-gray-400 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition">Cancel</button>
        <button type="submit" id="submitSalesBtn" class="px-6 py-2 rounded-md font-semibold bg-red-600 hover:bg-red-700 text-white transition">Save Sale</button>
      </div>
    </form>
  </div>
</div>
...
<div id="viewSalesModal"
  ...
  <div id="viewSalesContent"
    ...
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Sales Detail</h2>
      <button id="closeViewSalesModal" class="text-gray-400 hover:text-red-600 transition">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-4 text-gray-800 dark:text-white">
      <div>
        <p><strong>DATE:</strong> <span id="view_sales_date" class="text-blue-600"></span></p>
        <p><strong>STATUS:</strong> <span id="view_sales_status"
            class="inline-block text-xs font-semibold px-2 py-1 rounded-md"></span></p>
        <p><strong>INVOICE NO:</strong> <span id="view_sales_invoice"></span></p>
      </div>
      <div>
        <p><strong>PAYMENT STATUS:</strong> <span id="view_sales_payment_status"
            class="inline-block text-xs font-semibold px-2 py-1 rounded-md"></span></p>
        <p><strong>CUSTOMER:</strong> <span id="view_sales_customer"></span></p>
      </div>
    </div>
    ...
    <div class="overflow-x-auto mb-6 rounded-xl">
      <div class="scrollbar-hide overflow-auto max-h-80">
        <table class="min-w-full text-sm text-left text-gray-800 dark:text-white">
          <thead class="bg-gray-50 dark:bg-gray-700 text-xs text-gray-600 dark:text-gray-300 uppercase font-medium">
            <tr>
          ...
          <tbody id="viewSalesItems" class="divide-y divide-gray-100 dark:divide-gray-700">
            <!-- diisi JS -->
          </tbody>
        </table>
      </div>
    ...
    <div class="flex justify-end mb-6">
      <div class="bg-white dark:bg-gray-700 shadow-sm rounded-lg border border-gray-300 dark:border-gray-600 w-full sm:w-[400px] px-6 py-4 text-sm text-gray-800 dark:text-white">
        <div class="flex justify-between items-center border-b border-gray-200 dark:border-gray-600 pb-2 mb-2">
          ...
          <span id="view_sales_grand_total" class="font-semibold text-lg text-gray-900 dark:text-white">Rp0</span>
        </div>
        <div class="flex justify-between items-center mb-1">
          <span class="text-gray-500 dark:text-gray-400">Paid</span>
          <span id="view_sales_paid" class="font-medium">Rp0</span>
        </div>
        <div class="flex justify-between items-center">
          <span class="text-gray-500 dark:text-gray-400">Due</span>
          <span id="view_sales_due" class="font-medium text-red-600">Rp0</span>
        </div>
      </div>
    </div>
...
<!-- MODAL RETURN SALE -->
<div id="returnSalesModal"
  class="fixed inset-0 z-[60] hidden bg-black/50 backdrop-blur-sm flex items-center justify-center overflow-y-auto p-4">
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-4xl p-6 relative">

    <form id="returnSalesForm">
      <input type="hidden" id="return_sale_id" name="sale_id" />

      <!-- Header -->
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Return Sales</h2>
        <button id="closeReturnSalesModalBtn" type="button" class="text-gray-400 hover:text-red-600 transition">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="text-sm mb-4">
        <p class="font-bold">Sales Info:</p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          <div>
            <p>DATE: <span id="return_sale_date"></span></p>
            <p>STATUS: <span id="return_sale_status"
                class="inline-block text-xs font-semibold px-2 py-1 rounded-md bg-green-100 text-green-700"></span></p>
            <p>INVOICE NO: <span id="return_sale_invoice"></span></p>
          </div>
          <div>
            <p>PAYMENT STATUS: <span id="return_sale_payment_status"
                class="inline-block text-xs font-semibold px-2 py-1 rounded-md bg-red-100 text-red-700"></span></p>
            <p>CUSTOMER: <span id="return_sale_customer"></span></p>
          </div>
        </div>
      </div>
      ...
      <div class="w-full border border-gray-200 rounded-md overflow-hidden">
        <div class="grid grid-cols-12 bg-gray-100 text-xs font-medium text-gray-700 uppercase">
          ...
          <div class="col-span-2 p-2 text-center">Price</div>
          <div class="col-span-2 p-2 text-right">Subtotal</div>
          <div class="col-span-1 p-2 text-center"></div>
        </div>

        <div id="return_sales_items" class="divide-y divide-gray-100 max-h-[300px] overflow-y-auto">
          <!-- JS rows here -->
        </div>

        <div class="border-t border-gray-100 px-4 py-2">
          <button type="button" id="addReturnSalesRow"
      ...
      <div class="mt-4 flex justify-end text-lg font-bold text-gray-800 dark:text-white">
        <span class="mr-2">Total:</span>
        <span id="return_sales_grand_total">Rp 0</span>
      </div>

      <!-- Tombol -->
      <div class="flex justify-end gap-3 mt-4">
        <button type="button"
          class="cancel-return-sales-btn px-4 py-2 border rounded-md text-gray-700 dark:text-white border-gray-400 dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 transition text-sm font-medium">
          Cancel
        </button>
        <button type="submit"
...
<!-- MODAL FILTER SALES -->
<div id="salesFilterModal" class="fixed inset-0 z-50 hidden bg-black/50 backdrop-blur-sm flex items-center justify-center overflow-y-auto p-4">
  <div id="salesFilterContent" class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-3xl p-6 transform transition-all duration-300 scale-95 opacity-0">

    <!-- Modal Header -->
    <div class="flex justify-between items-center mb-4">
      <h3 class="text-xl font-semibold text-gray-800 dark:text-white">Filter Sales</h3>
      <button id="closeSalesFilterModalBtn" class="text-gray-500 hover:text-red-600 dark:hover:text-red-400">
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      <div>
        ...
        <input type="date" id="filterSalesStartDate" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" />
      </div>
      <div>
        <label class="block mb-1 text-sm">Tanggal Selesai</label>
        <input type="date" id="filterSalesEndDate" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" />
      </div>
      <div>
        <label class="block mb-1 text-sm">Customer</label>
        <select id="filterSalesCustomer" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white">
          <option value="">-- Semua Customer --</option>
        </select>
      </div>
      <div>
        <label class="block mb-1 text-sm">Status</label>
        <select id="filterSalesStatus" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white">
      ...
      <div>
        ...
        <select id="filterSalesPaymentStatus" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white">
          <option value="">-- Semua --</option>
          <option value="paid">Paid</option>
          <option value="partial">Partial</option>
          <option value="unpaid">Unpaid</option>
      ...
      <div>
        ...
        <select id="filterSalesInvoice" class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white">
          <option value="">-- Semua Invoice --</option>
        </select>
      </div>
      <div>
        <label class="block mb-1 text-sm">Grand Total (Min)</label>
        <input type="text" id="filterSalesGrandMin" placeholder="Minimal"
          class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" />
      </div>
      <div>
        <label class="block mb-1 text-sm">Grand Total (Max)</label>
        <input type="text" id="filterSalesGrandMax" placeholder="Maksimal"
          class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" />
      </div>
      <div>
        <label class="block mb-1 text-sm">Jumlah Item ≥</label>
        <input type="number" id="filterSalesMinItems" min="1" placeholder="Contoh: 1"
          class="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" />
      </div>
    </div>
    ...
    <div class="flex justify-between mt-6">
      <button id="clearSalesFilter" class="border border-red-600 text-red-600 hover:bg-red-50 dark:hover:bg-red-900 px-4 py-2 rounded">
        Clear Filter
      </button>
      <button id="applySalesFilter" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">
        Apply Filter
      </button>
    </div>
